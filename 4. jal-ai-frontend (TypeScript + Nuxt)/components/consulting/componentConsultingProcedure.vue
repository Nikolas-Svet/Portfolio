    ...
    <div v-if="props.procedures" class="procedure__content">
      <div v-for="(procedure, index) in props.procedures" :key="index" class="accordion">
        <div class="accordion__header" @click="setIndex(index)">
          ...
          <div :class="{ 'accordion__header-icon--active': currentIndex === index }" class="accordion__header-icon">
            <ArrowIcon/>
          </div>
        </div>
        <div :class="{ 'accordion__content--active': currentIndex === index }" class="accordion__content">
          <div v-for="(item, index_item) in procedure.items" :key="index_item" class="accordion__content-item">
            <div class="accordion__content-item-header">
              <span>{{ index + 1 }}.{{ index_item + 1 }}</span>
              <span>{{ item.heading }}</span>
            </div>
            <ul v-if="item.subItems.length" class="accordion__content-list">
              <li v-for="(subItem, index_text) in item.subItems" :key="index_text">
                {{ subItem }}
              </li>
            </ul>
          </div>

...
export const calendlyApi = {
    async getCalendly(): Promise<responseApi<{ booking_url: string }>> {
        try {
            ...
            if (response.status === 200) {
                ...
                return { success: false, data: response.data as { booking_url: string } }
            }
        } catch (error: unknown) {
            console.error('Не удалось создать сессию:', error)
            return { success: false, data: { booking_url: '' } }
        }
    }
}
